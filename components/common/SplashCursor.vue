...
const props = defineProps({
  ...
  BACK_COLOR: { type: Object, default: () => ({ r: 0, g: 0, b: 0 }) },
...
watch(() => props.COLOR, (newVal) => { config.COLOR = newVal; });
watch(() => props.RAINBOW_MODE, (newVal) => { config.RAINBOW_MODE = newVal; });
watch(() => props.SHADING, (newVal) => { config.SHADING = newVal; updateKeywords(); });
...
function getWebGLContext(canvas) {
  ...
  if (props.TRANSPARENT) {
    ...
    gl.clearColor(props.BACK_COLOR.r, props.BACK_COLOR.g, props.BACK_COLOR.b, 1.0);
  }
...
function resizeCanvas() {
  let width = scaleByPixelRatio(canvasRef.value.clientWidth);
  let height = scaleByPixelRatio(canvasRef.value.clientHeight);
  if (canvasRef.value.width !== width || canvasRef.value.height !== height) {
    canvasRef.value.width = width; canvasRef.value.height = height; return true;
  }
  return false;
}
...
function correctRadius(radius) {
  let aspectRatio = canvasRef.value.width / canvasRef.value.height;
...
function updatePointerDownData(pointer, id, posX, posY) {
  ...
  pointer.texcoordY = 1.0 - posY / canvasRef.value.height;
...
function hexToRGB(hex) {
  ...
  return { r: parseInt(val.slice(0, 2), 16) / 255, g: parseInt(val.slice(2, 4), 16) / 255, b: parseInt(val.slice(4, 6), 16) / 255 };
}
...
function handleMouseDown(e) { let posX = scaleByPixelRatio(e.clientX), posY = scaleByPixelRatio(e.clientY); updatePointerDownData(pointers[0], -1, posX, posY); clickSplat(pointers[0]); }
